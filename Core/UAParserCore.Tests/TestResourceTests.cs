...
namespace UAParserCore.Tests
{
  public class TestResourceTests
  {
    ...
    {
      RunTests("UAParserCore.Tests.TestResources.test_device.yaml", DeviceYamlTestCase.ReadFromMap);
    }

    [Fact]
    public void can_run_additional_os_tests()
    {
      RunTests("UAParserCore.Tests.TestResources.additional_os_tests.yaml", OSYamlTestCase.ReadFromMap);
    }

    [Fact]
    public void can_run_firefox_user_agent_string_tests()
    {
      RunTests("UAParserCore.Tests.TestResources.firefox_user_agent_strings.yaml", UserAgentYamlTestCase.ReadFromMap);
    }

    [Fact]
    public void can_run_pgts_browser_list_tests()
    {
      RunTests("UAParserCore.Tests.TestResources.pgts_browser_list.yaml", UserAgentYamlTestCase.ReadFromMap);
    }

    [Fact]
    public void can_run_user_agent_parser_tests()
    {
      RunTests("UAParserCore.Tests.TestResources.test_ua.yaml", UserAgentYamlTestCase.ReadFromMap);
    }

    [Fact]
    public void can_run_user_agent_parser_os_tests()
    {
      RunTests("UAParserCore.Tests.TestResources.test_os.yaml", OSYamlTestCase.ReadFromMap);
    }
    ...
    private static void RunTestCases<TTestCase>(List<TTestCase> testCases) where TTestCase : YamlTestCase
    {
      ...
      for (int i = 0; i < testCases.Count; i++)
      {
        ...
        catch (Xunit.Sdk.AssertActualExpectedException ex)
        {
